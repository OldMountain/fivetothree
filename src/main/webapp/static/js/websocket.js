...
//初始化事件
fttWebSocket.prototype.init = function(params){
...
fttWebSocket.prototype.bindMethod = function () {
    var thisWebSocket = this;
...
//向服务器发送数据
fttWebSocket.prototype.send = function(param){
    if(this.socket == null){
        logger.error(" fttWebSocket Not yet initialized");
        alert("fttWebSocket Not yet initialized");
    }else{
        var msg = "";
...
//关闭连接
fttWebSocket.prototype.close = function(){
    this.chonglian = false,
        this.socket.close();
}
//打开一个新的链接
fttWebSocket.prototype.open = function(){
    this.init();
    return this;
}
/** 打印输出log日志 */
fttWebSocket.prototype.printLog = function(msg) {
    if (this.outLog) {
        console.log(msg);
    }
}
